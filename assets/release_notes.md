...
- **Claude Code 작업 알림 통합**: 설정 > 알림 > Claude Code 섹션에서 [설치] 버튼으로 알림 브리지 플러그인을 등록하면 Claude Code의 작업 완료/입력 대기/권한 요청이 Dokkaebi 워크스페이스 알림으로 표시됨. "작업 알림" 토글로 표시 여부를 즉시 켜고 끌 수 있고, [제거] 버튼으로 언제든지 해제 가능. 인스톨러에 플러그인이 함께 번들되어 별도 다운로드 불필요
