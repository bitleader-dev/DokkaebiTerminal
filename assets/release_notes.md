...
### UI/UX 개선
- **알림 메뉴 재구성**: 기존 "Claude Code 작업 완료 알림음" 항목이 "Claude Code 플러그인" + "작업 알림" 두 항목으로 재구성. 플러그인 설치 상태가 화면에 즉시 표시되고, 알림 종류별로 자동으로 사라지거나(작업 완료 5초) 사용자가 닫을 때까지 유지(입력 대기·권한 요청)됨
