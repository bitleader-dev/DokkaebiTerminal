...
/// `ModelId` (예: "anthropic/claude-3-5-sonnet") 를 `LanguageModelSelection` 으로 변환한다.
/// 레지스트리에서 해당 모델을 찾을 수 있으면 현재 사용자 설정(thinking/effort/speed) 을
/// 존중한 상태로 생성하고, 찾을 수 없으면 플레인 식별자만 담는다.
