use crate::{
    ...
    PhantomDiffReviewIndicator, Point, RowExt, RowRangeExt,
...
impl EditorElement {
    ...
    pub(crate) fn mouse_moved(
        ...
        // Don't show diff review indicator when another indicator is active on this row
                            ...
                            v_flex()
                                ...
                                .child(
                                    h_flex()
                                        .gap_1()
                                        .justify_between()
                                        .child(Label::new(t("editor.tooltip.show_symbol_outline", cx)))
                                ...
                                .when(has_project_path, |this| {
                                    this.child(
                                        h_flex()
                                            ...
                                            .child(Label::new(t("editor.tooltip.right_click_copy_path", cx))),
                                    )
                                })
                                .into_any_element()
                        }))
                        ...
                        .when(can_open_excerpts && relative_path.is_some(), |this| {
                            this.child(
                                div()
                                    ...
                                    .child(
                                        Button::new("open-file-button", t("editor.button.open_file", cx))
        ...
        window.with_rem_size(rem_size, |window| {
            window.with_text_style(Some(text_style), |window| {
                window.with_content_mask(Some(ContentMask { bounds }), |window| {
                    ...
                    // sticky scroll header 아래 영역만 mask. transparent 배경에서
                    // editor 내용이 sticky header 뒤로 비쳐 보이지 않도록 차단.
...
pub struct PointForPosition {
    ...
    /// inlay bias 를 반영해 가까운 valid point 를 선택. inlay 와 인접한 위치에서 클릭 시
    /// 어색한 셀렉션 점프를 방지한다.
...
impl PositionMap {
    pub(crate) fn point_for_position(&self, position: gpui::Point<Pixels>) -> PointForPosition {
        ...
        let nearest_valid = self.pick_nearest_valid(exact_unclipped, previous_valid, next_valid);
    ...
    /// inlay 인접 클릭에서 어느 쪽 valid point 로 붙일지 결정한다.
    fn pick_nearest_valid(
        &self,
        ...
        match self.snapshot.inlay_bias_at(exact_unclipped) {
            ...
            Some(Bias::Right) | None => previous_valid,
        }
    }
}
