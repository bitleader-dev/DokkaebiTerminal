...
impl Render for MergeConflictIndicator {
    fn render(&mut self, _window: &mut Window, cx: &mut Context<Self>) -> impl IntoElement {
        ...
        // 상태바 라벨·툴팁 i18n 치환 (단수/복수 분기)
        let message_key = if file_count == 1 {
            "conflict_view.indicator.resolve_single"
        } else {
            "conflict_view.indicator.resolve_multi"
        };
        let tooltip_key = if file_count == 1 {
            "conflict_view.indicator.tooltip_single"
        } else {
            "conflict_view.indicator.tooltip_multi"
        };
        let message: SharedString = i18n::t(message_key, cx);
        let tooltip_label: SharedString = i18n::t_args(
            tooltip_key,
            &[("count", &file_count.to_string())],
            cx,
        )
        .into();
        let tooltip_meta: SharedString = i18n::t("conflict_view.indicator.tooltip_meta", cx);
