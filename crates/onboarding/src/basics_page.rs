...
use ui::{
    StatefulInteractiveElement, ToggleButtonGroup,
...
fn render_theme_section(tab_index: &mut isize, cx: &mut App) -> impl IntoElement {
    ...
    return v_flex()
        .gap_2()
        .child(
            h_flex().justify_between().child(Label::new(t("onboarding.theme", cx))).child({
                let mode_names: [SharedString; 3] = [
                    t("onboarding.theme.light", cx),
                    t("onboarding.theme.dark", cx),
                    t("onboarding.theme.system", cx),
                ];
...
fn render_base_keymap_section(tab_index: &mut isize, cx: &mut App) -> impl IntoElement {
    ...
    return v_flex().gap_2().child(Label::new(t("onboarding.base_keymap", cx))).child(
