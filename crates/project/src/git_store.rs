...
pub enum RepositoryEvent {
    StatusesChanged,
    /// HEAD 포인터(현재 브랜치 이름 또는 head commit)가 변경된 경우.
    HeadChanged,
    /// 로컬 브랜치 목록 자체가 변경된 경우(생성·삭제·이름 변경 등).
    BranchListChanged,
    StashEntriesChanged,
...
impl Repository {
    ...
    fn local(
        ...
        cx.subscribe_self(move |this, event: &RepositoryEvent, _| match event {
            ...
            RepositoryEvent::StashEntriesChanged => {
                // Stash 변경은 "모든 커밋 + stash" 뷰(LogSource::All)에서만
                // 그래프가 갱신되어야 하므로 해당 소스 캐시만 제거한다.
                if this.scan_id > 1 {
    ...
    pub fn create_worktree_detached(
        ...
        self.send_job(
            Some(format!("git worktree add --detach: {}", path.display()).into()),
            move |repo, _cx| async move {
                match repo {
                    RepositoryState::Local(LocalRepositoryState { backend, .. }) => {
                        backend.create_worktree_detached(path, commit).await
                    }
    ...
    pub fn checkout_branch_in_worktree(
        ...
        self.send_job(Some(description.into()), move |repo, _cx| async move {
            match repo {
                ...
                RepositoryState::Remote(_) => {
                    log::warn!(
                        "checkout_branch_in_worktree is not supported for remote repositories"
                    );
                    Ok(())
                }
            }
    ...
    pub fn create_archive_checkpoint(
        &mut self,
...
async fn compute_snapshot(
    ...
    let (branches, head_commit, all_worktrees) = cx
        ...
        .await?;
    // 현재 HEAD 브랜치 + 전체 브랜치 목록을 함께 스냅샷에 보관한다.
    // 브랜치 생성·삭제가 HEAD 변경과 무관하게 발생해도 그래프·UI 가
    // 즉시 갱신되도록 한다.
