...
/// 제목 텍스트로부터 URL 친화적인 슬러그를 생성한다 (예: "Hello World" → "hello-world").
...
/// URL이 URI 스킴으로 시작하는지 확인한다 (RFC 3986 §3.1).
...
/// 상대 URL을 경로와 `#fragment` 부분으로 분리한다.
/// 절대 URL은 그대로 반환하며 fragment는 없다.
