...
impl Vim {
    ...
    pub fn pane(&self, window: &Window, cx: &Context<Self>) -> Option<Entity<Pane>> {
        ...
        // focused_pane 은 자체 pane 이 없는 dock panel(예: Agent panel) 이 포커스를
        // 가졌을 때 center pane 으로 폴백한다. vim search/match 명령이
        // 그 폴백 경로로 포커스를 빼앗지 않도록 가드한다.
