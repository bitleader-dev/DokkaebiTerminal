...
impl BackgroundScannerState {
    ...
    fn remove_path(&mut self, path: &RelPath, watcher: &dyn Watcher) {
        ...
        for entry in removed_entries.cursor::<()>(()) {
            ...
            // symlink 가 사라졌으면 target 매핑에서도 제거 — 누적 누수 방지.
            if let Some(target) = entry.canonical_path.as_ref() {
                let key: Arc<Path> = target.clone();
                let became_empty = if let Some(paths) =
                    self.symlink_paths_by_target.get_mut(&key)
                {
                ...
                if became_empty {
                    self.symlink_paths_by_target.remove(&key);
                }
            }
...
impl BackgroundScanner {
    ...
    /// symlink 가 가리키는 외부 디렉터리에서 발생한 PathEvent 를 worktree
    /// 안의 symlink 경로로 매핑한 추가 이벤트로 변환한다.
    fn normalized_events_for_worktree(
        ...
        events.retain(|event| {
            ...
            // symlink 의 실제 타겟이 worktree 밖이면 원본 경로는 worktree 에 보이지 않으므로
            // remap 된 symlink 이벤트만 보존한다.
