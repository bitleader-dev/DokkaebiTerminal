...
fn files_not_created_on_launch(errors: HashMap<io::ErrorKind, Vec<&Path>>) {
    let message = "Dokkaebi failed to launch";
...
fn fail_to_open_window(e: anyhow::Error, _cx: &mut App) {
    eprintln!("Dokkaebi failed to open a window: {e:?}");
    ...
    #[cfg(any(target_os = "linux", target_os = "freebsd"))]
    {
        ...
        _cx.spawn(async move |_cx| {
            ...
            proxy
                .add_notification(
                    notification_id,
                    Notification::new("Dokkaebi failed to launch")
...
fn main() {
    ...
    app.run(move |cx| {
        ...
        // i18n 초기화 (번역 파일 로드 및 현재 로케일 설정)
        i18n::init(cx);

        // 로케일 변경 시 메뉴 실시간 갱신
        let mut prev_locale = i18n::current_locale(cx);
        cx.observe_global::<settings::SettingsStore>({
            move |cx| {
                let new_locale = i18n::I18nSettings::get_global(cx).locale;
                if new_locale != prev_locale {
                    prev_locale = new_locale;
                    i18n::set_locale(new_locale, cx);
...
async fn authenticate(_client: Arc<Client>, _cx: &AsyncApp) -> Result<()> {
    // Dokkaebi는 Zed cloud 로그인을 지원하지 않는다.
    // 앱 시작 시 저장된 크리덴셜로의 자동 재로그인 경로를 차단한다.
    Ok(())
}
...
struct Args {
    ...
    ///
    /// URLs can either be `file://` or `zed://` scheme.
