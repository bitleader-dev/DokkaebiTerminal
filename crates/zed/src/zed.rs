...
fn initialize_panels(
    ...
    cx.spawn_in(window, async move |workspace_handle, cx| {
        ...
        let notepad_panel = notepad_panel::NotepadPanel::load(workspace_handle.clone(), cx.clone());
        ...
        futures::join!(
            ...
            add_panel_when_ready(notepad_panel, workspace_handle.clone(), cx.clone()),
...
fn reload_keymaps(cx: &mut App, mut user_key_bindings: Vec<KeyBinding>) {
    ...
    cx.set_dock_menu(vec![gpui::MenuItem::action(
        i18n::t("menu.file.new_window", cx),
        workspace::NewWindow,
    )]);
    // todo: nicer api here?
...
mod tests {
    ...
    async fn test_window_edit_state_restoring_enabled(cx: &mut TestAppContext) {
        ...
        assert!(
            ...
            "복원된 워크스페이스는 안정적인 database id 를 가져야 한다"
        );
        ...
        assert_eq!(
            ...
            initial_database_id,
            "워크스페이스가 close/reopen 사이클에서 동일한 database id 를 유지해야 한다"
        );
